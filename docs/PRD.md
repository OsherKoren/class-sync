...
- **Class Recurrence:** Teacher sets class as **recurring** (runs weekly indefinitely until cancelled) or **one-time** (a single session — e.g. a special makeup or trial lesson)
...
- **Enrollment Type:** When enrolling, the student or guardian chooses **recurring** (attends every session of the class going forward) or **one-time** (attends a single specific session — for trials, makeups, or covering a missed week in their regular class)
...
- **One-Time Enrollment:** Student or guardian can enroll in a single specific session of any open class (e.g. joining another group's session to cover a missed week); capacity is checked at the session level so temporary attendees count toward the cap for that date only
...
- **Optional Phone Number:** Users add their phone in settings; no phone = no WhatsApp messages, no errors
- **Explicit Opt-In:** Users toggle WhatsApp notifications on/off (legally required for business messaging)
- **Event Triggers:** Reschedule offer, session cancellation, enrollment approved/rejected, student join request, 24h reminder
- **Fan-Out:** Class-level events notify all enrolled students and their linked guardians
- **Unified Helper:** Push + WhatsApp dispatched together from `lib/notifications.ts` so triggers never diverge

### Phase 8 — Bilingual Support (Hebrew/English)
...
- ✅ Push notifications
- ✅ WhatsApp notifications (Twilio, opt-in)
...
- **Class** — Tutoring session template (name, subject, day, time, duration, teacher, isOpen flag). Has an `isRecurring` flag: `true` = runs weekly indefinitely; `false` = one-time special session.
- **Enrollment** — Student + Class relationship with status (PENDING, ACTIVE, REJECTED) and a type (RECURRING or ONE_TIME).
  ...
  - RECURRING enrollment: student attends every future session of the class.
  - ONE_TIME enrollment: student attends a single specific `LessonSession` (stored as `lessonSessionId` on the enrollment). Capacity for that session is checked against both recurring enrollees and any one-time attendees for that date.
...
| 6 | Push & PWA | Planned |
| 7 | WhatsApp notifications (Twilio) | Planned |
| 8 | i18n (Hebrew/English) | Planned |
| 9 | Dark mode | Planned |
| 10 | Deploy & polish | Planned |
| 11 | AI assistant (v2) | Post-launch |
